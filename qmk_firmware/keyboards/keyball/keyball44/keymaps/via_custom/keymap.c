...
// Tap Dance 識別子（TD()で使われている定数をここで定義）
enum {
  TD_KANA_EISU,
};
...
// コンボアクションへの割り当て（配列の終わりを示す COMBO_END/NULL は不要）
combo_t key_combos[] = {
...
void process_combo_event(uint16_t combo_index, bool pressed) {
  if (pressed) {
    switch (combo_index) {
      ...
      // --- ドラッグ・トグル用コンボ ---
    case KL_BTN1: // • K + L -> Btn1 (トグル)
      toggle_mouse_btn1();
      break;
    case LS_BTN2: // • L + ; -> Btn2 (トグル)
      // • Btn1 + Btn2 -> Btn2 のロジックもここでカバー
      toggle_mouse_btn2();
      break;
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
  ...
  if (record->event.pressed) {
    ...
    // • 9 + Btn1 -> Btn1 (トグル)
    if (keycode == KC_9) {
      // Btn1がロック中ならトグル（解除/再ロック）
      if (is_btn1_locked) {
        toggle_mouse_btn1();
        // 9の入力自体はキャンセルしない (移動も継続させるため)
      }
    }
    ...
    // • Btn1 + Btn2 -> Btn2 の特殊処理は process_combo_event でカバー済
    // (Btn1ロック中にBtn2コンボを押した場合)
